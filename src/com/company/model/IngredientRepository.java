...
public class IngredientRepository implements IRepository {
    ...
    public boolean create(Object entity) {
        ...
        return dbConnector.insert("INSERT INTO `zutaten`(`name`, `vegetarisch`, `preis`) VALUES " +
                "('" + ingredient.getName() + "', " + ingredient.isVegetarian() + ", " + ingredient.getSinglePrice() + ")");
    }

    public boolean delete (int id) {
        return dbConnector.delete("DELETE FROM `zutaten` WHERE `id` = " + id);
    }

    public boolean updatePrice (int id, double price){
        return dbConnector.update("UPDATE `zutaten` SET `preis`= " + price + " WHERE `id` = " + id);
    }
}
